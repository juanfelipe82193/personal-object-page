/*!
 * OpenUI5
 * (c) Copyright 2009-2021 SAP SE or an SAP affiliate company.
 * Licensed under the Apache License, Version 2.0 - see LICENSE.txt.
 */

@import "../../../../sap/ui/core/themes/base/base.less";
@import "../../../../sap/ui/core/themes/base/global.less";
@import "shared.less";

@import "ActionListItem.less";
@import "ActionSelect.less";
@import "ActionSheet.less";
@import "App.less";
@import "Avatar.less";
@import "Bar.less";
@import "BarBase.less";
@import "Breadcrumbs.less";
@import "BusyDialog.less";
@import "BusyIndicator.less";
@import "Button.less";
@import "Carousel.less";
@import "CheckBox.less";
@import "ColumnListItem.less";
@import "ColorPalette.less";
@import "CustomTile.less";
@import "DateRangeSelection.less";
@import "DatePicker.less";
@import "DateTimePicker.less";
@import "Dialog.less";
@import "DisplayListItem.less";
@import "DraftIndicator.less";
@import "ExpandableText.less";
@import "MessagePage.less";
@import "FacetFilter.less";
@import "FacetFilterItem.less";
@import "FacetFilterList.less";
@import "FeedContent.less";
@import "FeedInput.less";
@import "FeedListItem.less";
@import "FlexBox.less";
@import "FormattedText.less";
@import "GenericTag.less";
@import "GenericTile.less";
@import "GroupHeaderListItem.less";
@import "HeaderContainer.less";
@import "IconTabBar.less";
@import "IconTabBarSelectList.less";
@import "Image.less";
@import "ImageContent.less";
@import "Input.less";
@import "InputBase.less";

/* note: the following three files are not in alphabetical order and should be imported after InputBase.less */
@import "ComboBoxTextField.less";
@import "ComboBoxBase.less";
@import "ComboBox.less";

@import "InputListItem.less";
@import "Label.less";
@import "LightBox.less";
@import "Link.less";
@import "ListBase.less";
@import "ListItemBase.less";

/* Note: The following file is not in alphabetical order and should be imported after ListItemBase.less */
@import "CustomListItem.less";

@import "Menu.less";
@import "MenuButton.less";
@import "MenuListItem.less";
@import "MessageBox.less";
@import "MessageStrip.less";
@import "MessagePopover.less";
@import "MessageView.less";
@import "MessageToast.less";
@import "MultiComboBox.less";
@import "MultiInput.less";
@import "NavContainer.less";
@import "NewsContent.less";
@import "NotificationListBase.less";
@import "NotificationListGroup.less";
@import "NotificationListItem.less";
@import "NumericContent.less";
@import "ObjectAttribute.less";
@import "ObjectHeader.less";
@import "ObjectIdentifier.less";
@import "ObjectListItem.less";
@import "ObjectMarker.less";
@import "ObjectNumber.less";
@import "ObjectStatus.less";
@import "OverflowToolbarAssociativePopover.less";
@import "P13nColumnsPanel.less";
@import "P13nConditionPanel.less";
@import "P13nDialog.less";
@import "P13nFilterPanel.less";
@import "P13nSortPanel.less";
@import "Page.less";
@import "PagingButton.less";
@import "Panel.less";
@import "PDFViewer.less";
@import "PlanningCalendar.less";
@import "PlanningCalendarHeader.less";
@import "PlanningCalendarLegend.less";
@import "Popover.less";
@import "ProgressIndicator.less";
@import "PullToRefresh.less";
@import "QuickView.less";
@import "RadioButton.less";
@import "RadioButtonGroup.less";
@import "RangeSlider.less";
@import "RatingIndicator.less";
@import "ScrollBar.less";
@import "ScrollContainer.less";
@import "SearchField.less";
@import "SegmentedButton.less";
@import "Select.less";
@import "SelectDialog.less";
@import "SelectList.less";
@import "SelectionDetails.less";
@import "SelectionDetailsItem.less";
@import "SemanticPage.less";
@import "SimpleFixFlex.less";
@import "SuggestionsPopover.less";
@import "ValueStateHeader.less";
@import "Shell.less";
@import "SinglePlanningCalendar.less";
@import "SinglePlanningCalendarGrid.less";
@import "SinglePlanningCalendarMonthGrid.less";
@import "Slider.less";
@import "SliderTooltip.less";
@import "SliderTooltipContainer.less";
@import "SlideTile.less";
@import "SplitButton.less";
@import "SplitContainer.less";
@import "StandardListItem.less";
@import "StandardTreeItem.less";
@import "StandardTile.less";
@import "StepInput.less";
@import "SuggestionItem.less";
@import "Support.less";
@import "Switch.less";
@import "TabContainer.less";
@import "Table.less";
@import "TablePersoDialog.less";
@import "TableSelectDialog.less";
@import "TabStrip.less";
@import "Text.less";
@import "TextArea.less";
@import "Tile.less";
@import "TileContent.less";
@import "TileContainer.less";
@import "TimePicker.less";
@import "TimePickerInputs.less";
@import "TimePickerClock.less";
@import "TimePickerClocks.less";
@import "TimePickerSliders.less";
@import "Title.less";
@import "ToggleButton.less";
@import "Token.less";
@import "Tokenizer.less";
@import "Toolbar.less";
@import "ToolbarSeparator.less";
@import "ToolbarSpacer.less";
@import "TreeItemBase.less";
@import "UploadCollection.less";
@import "UploadSet.less";
@import "ValueStateMessage.less";
@import "ViewSettingsDialog.less";
@import "WheelSlider.less";
@import "WheelSliderContainer.less";
@import "Wizard.less";
@import "WizardProgressNavigator.less";
@import "WizardStep.less";

/* import plugin less files */
@import "ColumnResizer.less";
@import "PasteProvider.less";